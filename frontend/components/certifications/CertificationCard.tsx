...
export function CertificationCard({ cert }: { cert: Certification }) {
  ...
  return (
    <motion.li
      variants={fadeUp}
      className="group flex h-full flex-col overflow-hidden rounded-[4px] border border-white/[0.08] bg-surface transition-all duration-300 hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-glow-soft"
    >
      <div className="relative aspect-[4/3] overflow-hidden border-b border-white/[0.08] bg-base">
        ...
        <span className="absolute left-3 top-3 grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-base shadow-card ring-1 ring-white/10">
          {BadgeIcon ? (
            ...
            <GenericBadgeIcon letter={cert.issuer[0]} width={22} height={22} />
          )}
        </span>
      </div>
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-3 font-mono text-xs text-accent">{cert.date}</p>
